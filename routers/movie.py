...
async def create_movie_endpoint(request: schemas.Movie, db: AsyncIOMotorDatabase = Depends(get_db), get_current_user: schemas.User = Depends(oauth2.get_current_user)):
...
async def update_movie_endpoint(movie_id: str, request: schemas.UpdateMovie, db: AsyncIOMotorDatabase = Depends(get_db), get_current_user: schemas.User = Depends(oauth2.get_current_user)):
...
async def delete_movie_endpoint(movie_id: str, db: AsyncIOMotorDatabase = Depends(get_db), get_current_user: schemas.User = Depends(oauth2.get_current_user)):
